...
- 全新轻量视觉体系：统一分层表面、圆角、焦点反馈与紫灰色调，浅色、深色和黑白模式均保持清晰对比度
- 重新设计主工具栏、地址栏、横向/垂直标签页、状态栏与启动页；宽屏下信息密度更合理，同时不引入额外 UI 框架
- 可在简体中文、繁体中文、英语、日语、韩语、葡萄牙语、西班牙语、俄语、法语和德语之间切换界面语言
- 横向或可折叠垂直标签页；支持 favicon、复制、关闭其他/右侧标签，以及带平滑位移动画的实时拖拽排序
- Edge 风格工作区：可创建、重命名、删除和切换独立标签集合，也可把当前标签移入其他工作区；工作区与页面分组会跨重启恢复
...
- 书签分组、编辑和筛选，支持单独控制书签是否显示在主页；资料库、历史记录、启动页和标签栏统一显示网站图标
- 网站图标按完整来源隔离缓存；跨站导航不会沿用上一站图标，未访问书签会直接从目标站点补取 favicon，无可靠图标时使用简洁站点首字母占位
...
- 历史、书签和工作区页面列表使用 Windows DPAPI（当前用户）加密保存；2.1.0 首次读取时自动迁移并擦除旧明文历史/书签 JSON
...
自 2.1.0 起，正常退出会先停止导航、完成清除策略、释放全部 WebView2 控制器，并等待内核子进程退出，然后才让窗口消失。请在窗口完全关闭后再安全弹出移动存储；程序运行中直接拔盘或系统突然断电属于物理中断，任何应用都无法保证此时的缓存和数据库写入完整。
