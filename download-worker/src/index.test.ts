...
const KEY = "releases/v2.2.2/ZZZ-v2.2.2-win-x64.exe";
...
describe("download worker", () => {
  it("returns JSON health information at the root", async () => {
    ...
    expect(await response.json()).toMatchObject({ status: "ok", latest: "v2.2.2" });
  });
  ...
  it.each(["x64", "x86", "arm64"])("redirects /latest/%s to v2.2.2", async (architecture) => {
    const { env } = testEnv();
    const response = await handleRequest(
